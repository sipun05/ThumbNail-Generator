...
import { aspectRatios, type AspectRatio } from '../assests/assets/assets'
const  AspectRatioSelector = ({value,onChange} : {value:AspectRatio; onChange: (ratio : AspectRatio)=>void}) => {
    const iconMap ={
        '16:19' : <RectangleHorizontal className='size-6'/>,
    ...
    <div className='space-y-3 dark'>
        ...
        {aspectRatios.map((ratio)=>{
            ...
            return (
                ...
                className={`flex items-center gap-2 rounded-md border px-5 py-2.5 text-sm transition border-white/10 ${selected? 'bg-white/10' : 'hover:bg-white/5'}`}>
