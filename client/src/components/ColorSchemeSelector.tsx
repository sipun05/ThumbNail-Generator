import { colorSchemes } from "../assests/ColorSchemes";
        ...
        <div className='grid grid-cols-6 gap-3'>
          {
            colorSchemes.map((scheme)=>(
                <button className ={`relative rounded-lg transition-all ${value === scheme.id && 'ring-2 ring-pink-500'}`} title={scheme.name} key={scheme.id} onClick={()=>onChange(scheme.id)}>
